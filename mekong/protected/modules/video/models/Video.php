...
class Video extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('record_id, postdate, pic_thumb, pic_full, pic_desc, record_order, hot, specials, field1, field2, field3, field4, enable, hoiit_module_item_cat_cat_id', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function relations() {
        ...
        return array(
            'VideoCat' => array(self::BELONGS_TO, 'VideoCat', 'hoiit_module_item_cat_cat_id'),
            'VideoLanguage' => array(self::HAS_MANY, 'VideoLanguage', 'record_id', 'index' => 'language_id'),
        );
    }
    ...
    public function detailItem($tag) {
        $id = VideoLanguage::model()->getIDByTag($tag);
        $criteria = new CDbCriteria();
        $criteria->condition = 'enable=1';
    ...
    private function deleteRecord($id) {
        VideoLanguage::model()->deleteRecord($id);
    ...
    public function saveRecord($model, $id = null) {
        if (Yii::app()->controller->action->id == 'add') {
            ...
            $this->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('video_width_thumb'), Config::getValue('video_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']] . '-thumb');
            //$this->pic_full = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_full'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_full'], Config::getValue('video_width_full'), Config::getValue('video_height_full'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]);
            if (isset($_FILES[__CLASS__ . 'Form']['name']['pic_desc'])) {
                $this->pic_desc = implode("|", $file->uploadMulti($_FILES[__CLASS__ . 'Form']['name']['pic_desc'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_desc'], Config::getValue('video_width_desc'), Config::getValue('video_height_desc'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]));
            }
            ...
            Yii::import('ext.SimpleImage.CSimpleImage');
            $file = new CSimpleImage();
            $item->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('video_width_thumb'), Config::getValue('video_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']] . '-thumb', $item->pic_thumb);
            //$item->pic_full = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_full'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_full'], Config::getValue('video_width_full'), Config::getValue('video_height_full'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']], $item->pic_full);
            //upload pic_desc
            if (isset($_FILES[__CLASS__ . 'Form']['name']['pic_desc'])) {
                $uploaded = $file->uploadMulti($_FILES[__CLASS__ . 'Form']['name']['pic_desc'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_desc'], Config::getValue('video_width_desc'), Config::getValue('video_height_desc'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]);
        ...
        VideoLanguage::model()->saveRecord($id, $model);
    }

    //Back end - Get record to Edit
    public function loadEdit($id) {
        $command = Yii::app()->db->createCommand('SELECT pic_thumb, pic_full, pic_desc, hot, enable, hoiit_module_item_cat_cat_id FROM ' . $this->tableName() . ' WHERE record_id=:id');
