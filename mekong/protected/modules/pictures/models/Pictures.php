...
class Pictures extends CActiveRecord {
    ...
    public function relations() {
        ...
        return array(
            'PicturesCat' => array(self::BELONGS_TO, 'PicturesCat', 'hoiit_module_item_cat_cat_id'),
            //'Language' => array(self::MANY_MANY, 'Language', 'hoiit_module_Pictures_languages(record_id, language_id)'),
            'PicturesLanguage' => array(self::HAS_MANY, 'PicturesLanguage', 'record_id', 'index' => 'language_id'),
        );
    }
    ...
    public function detailItem($tag) {
        $id = PicturesLanguage::model()->getIDByTag($tag);
        $criteria = new CDbCriteria();
        $criteria->condition = 'enable=1';
    ...
    public function listItemAdmin() {
        $criteria = new CDbCriteria();
        //$criteria->with = array('Language', 'PicturesLanguage');
        $criteria->order = 'record_order DESC, postdate DESC';
    ...
    private function deleteRecord($id) {
        PicturesLanguage::model()->deleteRecord($id);
    ...
    public function saveRecord($model, $id = null) {
        if (Yii::app()->controller->action->id == 'add') {
            ...
            $this->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('pictures_width_thumb'), Config::getValue('pictures_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']] . '-thumb');
            $this->pic_full = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_full'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_full'], Config::getValue('pictures_width_full'), Config::getValue('pictures_height_full'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]);
            if (isset($_FILES[__CLASS__ . 'Form']['name']['pic_desc'])) {
                $this->pic_desc = implode("|", $file->uploadMulti($_FILES[__CLASS__ . 'Form']['name']['pic_desc'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_desc'], Config::getValue('pictures_width_desc'), Config::getValue('pictures_height_desc'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]));
            }
            ...
            Yii::import('ext.SimpleImage.CSimpleImage');
            $file = new CSimpleImage();
            $item->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('pictures_width_thumb'), Config::getValue('pictures_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']] . '-thumb', $item->pic_thumb);
            $item->pic_full = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_full'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_full'], Config::getValue('pictures_width_full'), Config::getValue('pictures_height_full'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']], $item->pic_full);
            //upload pic_desc
            if (isset($_FILES[__CLASS__ . 'Form']['name']['pic_desc'])) {
                $uploaded = $file->uploadMulti($_FILES[__CLASS__ . 'Form']['name']['pic_desc'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_desc'], Config::getValue('pictures_width_desc'), Config::getValue('pictures_height_desc'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]);
        ...
        PicturesLanguage::model()->saveRecord($id, $model);
    }
