...
/**
 * This is the model class for table "hoiit_module_services".
 *
 * The followings are the available columns in table 'hoiit_module_services':
...
class Services extends CActiveRecord {
    ...
    public function relations() {
        ...
        return array(
            'Language' => array(self::MANY_MANY, 'Language', 'hoiit_module_services_languages(record_id, language_id)'),
            'ServicesLanguage' => array(self::HAS_MANY, 'ServicesLanguage', 'record_id', 'index' => 'language_id'),
        );
    }
    ...
    public function firstRecord() {
        ...
        if ($viewer) {
            //update hit view
            ServicesLanguage::model()->updateCounters(array('hit' => 1), 'record_id=:id AND language_id=:lang', array(':id' => $viewer->record_id, ':lang' => Yii::app()->language));
            return $viewer->ServicesLanguage[Yii::app()->language];
        }
    }
    ...
    public function listItem() {
        $command = Yii::app()->db->createCommand('SELECT title, tag FROM ' . $this->tableName() . ', hoiit_module_services_languages WHERE ' . $this->tableName() . '.record_id = hoiit_module_services_languages.record_id AND language_id =\'' . Yii::app()->language . '\' AND hot = 0 AND enable = 1 ORDER BY record_order DESC, created DESC');
            ...
            if (!empty($ids)) {
                ...
                if ($flag) {
                    ...
                    foreach ($record_id as $value) {
                        $id = intval($value);
                        if ($id) {
                            ServicesLanguage::model()->deleteRecord($id);
                            $this->deleteByPk($id);
                        }
                    }
                }
