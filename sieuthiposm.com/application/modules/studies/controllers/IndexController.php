...
class Studies_IndexController extends GLT_Controller_Action {
	...
	public function catAction() {
		$itemCat=new Studies_Model_StudiesCat();
		$cid=$this->_data['cid'];	
		while ($cid !=0 ) {
		...
		$this->webTitle($this->view->itemCat[0].' - '.$this->view->lang[$this->_data['module']]);
	}
	...
	public function viewAction(){
		...
		$itemCat=new Studies_Model_StudiesCat();
		$cid=$detail['dos_module_studies_cat_cat_id'];	
		while ($cid !=0 ) {
			$listCatParent[] =	$itemCat->getDetailCatFromCid($cid);
			$pcid=$itemCat->getCatParent_CatOrder($cid);
			$cid=$pcid['cat_parent_id'];
		...
		if($this->_request->isPost()){	
			$validate = new Studies_Form_ValidateComment($this->_data);
