<?php
class Studies_Model_Studies extends Zend_Db_Table{
	protected $_name = 'dos_module_studies';
	...
	public function listNewsnew(){
    	...
    	$select = $db->select()->from(array('n' => $this->_name), array('record_id', 'pic_thumb', 'postdate', 'title'.LANG, 'preview'.LANG))
							   ->join(array('c' => $this->_name.'_cat'),'n.dos_module_studies_cat_cat_id = c.cat_id', array('cat_title'.LANG))
							   ->where('enable = 1')
							   ->order('record_order ASC')
							   ->order('postdate DESC')
							   ->limitPage(0, 3);
	...
	public function listNewsbyCat($data = NULL){
    	...
    	$select = $db->select()->from($this->_name, array('record_id', 'pic_thumb', 'postdate', 'title'.LANG, 'preview'.LANG))
							   ->where('enable = 1')
							   ->where('dos_module_studies_cat_cat_id =?', $data['cid'])
							   ->order('record_order ASC')
							   ->order('postdate DESC')
							   ->limitPage($page, $rowCount);
		$result = $db->query($select)->fetchAll();
    ...
    public function countItembyCatID($cid){
		...
		$select = $db->select()->from($this->_name, array('COUNT(record_id) AS num'))
                               ->where('dos_module_studies_cat_cat_id = ?', $cid);
        return $db->fetchOne($select);
    }
	...
	public function menuAdmin(){
        ...
        $menu[] = array('url'=>'studies/cat', 'name'=>'Danh mục', 'nameen'=>'Category', 'namefr'=>'Category', 'num'=>$cat->countCat());
        $menu[] = array('url'=>'studies', 'name'=>'Bài viết', 'nameen'=>'Post', 'namefr'=>'Post', 'num'=> $this->countItemadmin());
        $menu[] = array('url'=>'studies/comment', 'name'=>'Comment', 'nameen'=>'Comment', 'namefr'=>'Comment', 'num'=> 0);
        return $menu;
    }
	...
	public function delPic($id){
		...
		if($result['pic_thumb']){
			@unlink(PUBLIC_PATH.'/userfiles/images/studies/'.$result['pic_thumb']);
		}
    }
	...
	public function addItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
    		...
    		$upload->upload($_FILES['file_thumb'], $this->_config['studies_width_thumb'], $this->_config['studies_height_thumb'], $this->_config['sys_size_upload'], PUBLIC_PATH.'/userfiles/images/studies/');
	...
	public function saveItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
	    	...
	    	if($upload->isVail() == false){
				$file_thumb = $upload->transferFile(); //upload file
				$upload->removeFile(PUBLIC_PATH.'/userfiles/images/studies/'.$data['fileold_thumb']);
			}
    	}
	...
	public function delItembyCat($data = NULL){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id'))->where('dos_module_studies_cat_cat_id =?', $data['id']);
		$result = $db->query($select)->fetchAll();
    	...
    	if($data['delproduct'] == 'del'){
	    	...
	    	foreach ($result as $value){
	    		//move item
				$where = 'record_id = '.$value['record_id'];
		    	$this->update(array('dos_module_studies_cat_cat_id' => $data['catmove']), $where);
			}
    	}
    ...
    public function deleteItem($cat_id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id'))->where('dos_module_studies_cat_cat_id = ?', $cat_id);
		$result = $db->fetchAll($select);
		foreach ($result as $value){
			$this->delPic($value['record_id']);
		}
		$where = 'dos_module_studies_cat_cat_id = '.$cat_id;
		$this->delete($where);
    }
	//Back end - update item to new cat
    public function updateItem($cat_id, $cat_id_new){
    	$where = 'dos_module_studies_cat_cat_id = '.$cat_id;
		$this->update(array('dos_module_studies_cat_cat_id' => $cat_id_new), $where);
    }
}