<?php
class Marketing_IndexController extends GLT_Controller_Action {
    private $_data;
    protected $_config;
	...
	public function catAction() {
		$itemCat=new Marketing_Model_MarketingCat();
		$cid=$this->_data['cid'];	
		while ($cid !=0 ) {
	...
	public function viewAction(){
		...
		$itemCat=new Marketing_Model_MarketingCat();
		$cid=$detail['dos_module_marketing_cat_cat_id'];	
		while ($cid !=0 ) {
			$listCatParent[] =	$itemCat->getDetailCatFromCid($cid);
			$pcid=$itemCat->getCatParent_CatOrder($cid);
			$cid=$pcid['cat_parent_id'];
		...
		if($this->_request->isPost()){	
			$validate = new Marketing_Form_ValidateComment($this->_data);
