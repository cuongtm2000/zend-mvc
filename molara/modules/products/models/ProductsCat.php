    ...
    public function beforeSave() {
        ...
        if ($this->isNewRecord) {
            ...
            if ($_FILES['ProductsCat']['name']['pic_full']) {
                ...
                $this->pic_full = EUploadedImage::getInstance($this, 'pic_full')->processUpload(Configs::configTemplate('products_cat_width', Yii::app()->session['template']), Configs::configTemplate('products_cat_height', Yii::app()->session['template']), USERFILES . '/productsCat', $this->cat_title);
            }
        } else {
            //check file old and upload
            if ($_FILES['ProductsCat']['name']['pic_full']) {
                Yii::import('ext.EUploadedImage.EUploadedImage');
                $this->pic_full = EUploadedImage::getInstance($this, 'pic_full')->processUpload(Configs::configTemplate('products_cat_width', Yii::app()->session['template']), Configs::configTemplate('products_cat_height', Yii::app()->session['template']), USERFILES . '/productsCat', $this->cat_title, $this->_oldImage_full);
            } else {
