...
class Product_Model_Product extends Zend_Db_Table{
	...
	//-----------------------------------------
	public function listPostedItem(){
		$auth=Zend_Auth::getInstance();
		if($auth->hasIdentity())
			$user=$auth->getIdentity()->username;
	
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id', 'postdate', 'pic_thumb', 'enable','title'.LANG, 'hits'))
    						   ->where('dos_sys_users_username=?',$user)
							   ->order('postdate DESC');
		return $db->query($select)->fetchAll();
    }
	...
	//Ninh thêm 
		public function addItem2($data = NULL){
		//Upload multi file
		$file_desc_multi = array();
		$file_desc = $_FILES['file_desc'];
		$desc_name = $file_desc['name'];
    ...
    public function productByCat($data = NULL){
    	...
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id', 'pic_thumb', 'title'.LANG))
    ...
    public function productByCatNoneid($cid, $id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id', 'pic_thumb', 'title'.LANG, 'unit'))->where('dos_module_product_cat_cat_id =?', $cid)->where('record_id NOT IN (?)', $id)->where('enable = 1');
		$result = $db->query($select)->fetchAll();
    ...
    public function getDetail($id){
		$db = Zend_Registry::get('connectDb');
		$select = $db->select()->from($this->_name, array('record_id', 'pic_thumb', 'pic_full', 'pic_desc', 'title'.LANG, 'detail'.LANG, 'extra_field1', 'extra_field2', 'extra_field3', 'extra_field4', 'dos_module_product_cat_cat_id'))
                               ->where('enable = ?', 1)
                               ->where('record_id = ?', $id);
