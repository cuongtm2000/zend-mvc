...
class IndexController extends GLT_Controller_Action {
	...
	public function indexAction() {
		$this->view->headScript()->appendFile($this->_request->getBaseUrl().TEMPLATE.'/js/jcarousellite.js');
       	$this->view->headScript()->appendFile($this->_request->getBaseUrl().TEMPLATE.'/js/special-block.js');
       	
		$news = new News_Model_News();
		$this->view->news_first = $news->getItemFirst();
		$this->view->news_other = $news->listItemother();
		
		$news = new Handbook_Model_Handbook();
		$this->view->handbook_first = $news->getItemFirst();
		$this->view->handbook_other = $news->listItemother();
