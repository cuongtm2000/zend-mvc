...
class Banners extends CActiveRecord {
	...
	public function rules() {
		...
		return array(
			...
			array('banner_name, banner_url', 'length', 'max' => 45),
			...
			array('banner_id, banner_date, banner_name, banner_url, banner_link, banner_order, banner_type, position, enable', 'safe', 'on' => 'search'),
		);
	}
	...
	public function listItemAdmin() {
		...
		return array('models' => Banners::model()->findAll($criteria), 'pages' => $pages);
	}

	//Back end - position Banner
	public function positionBanner() {
		$model = new ModulesUsernames();
	...
	private function deleteRecord($id) {
		$item = Banners::model()->find('banner_id=:id', array(':id' => $id));
		...
		Banners::model()->findByPk($id)->delete(); //delete record_id
	}
	...
	public function activeItem($data) {
		...
		if ($sort) {
			...
			foreach ($models as $model) {
				Banners::model()->updateByPk($model['banner_id'], array('banner_order' => $i));
				$i++;
			}
		} else {
			if (!empty($ids)) {
	...
	public function loadEdit($id) {
		...
		$this->_model = Banners::model()->findByPk($id, $criteria);
