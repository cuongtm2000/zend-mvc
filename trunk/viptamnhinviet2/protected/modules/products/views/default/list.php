	...
	<table>
		...
		<?php if($models):?>
			<?php $i=1; foreach($models as $model):?>
				<?php if($model['enable'] == 0){$rowclass = ' class="disable"';}else{$rowclass = ($i%2==0) ? '' : ' class="alt"';} if($model['hot'] == 1){$title = '<strong>'.$model['title'].'</strong> <img src="'.Yii::app()->theme->baseUrl.'/images/hot.gif" alt="Hot" />';}else{$title = $model['title'];}?>
			...
			<?php $i++; endforeach?>
		<?php else:?>
				<tr><td colspan="5">Bạn chưa có sản phẩm</td></tr>
		<?php endif;?>
		</tbody>
	</table>
