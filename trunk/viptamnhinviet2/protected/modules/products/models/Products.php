 ...
 * @property DosModuleUsernames $dosModuleUsernamesUsername
 */
class Products extends CActiveRecord {
	...
	public function relations() {
		...
		return array(
			...
			//'dosModuleUsernamesUsername' => array(self::BELONGS_TO, 'DosModuleUsernames', 'dos_module_usernames_username'),
		);
	}
	...
	public function attributeLabels() {
		return array(
			...
			'enable' => 'Enable',
			'dos_module_item_cat_cat_id' => 'Phân loại',
	...
	public function beforeSave() {
		...
		if ($this->isNewRecord) {
			...
			if ($_FILES['Products']['name']['pic_thumb']) {
				...
				$this->pic_thumb = $file->processUpload($_FILES['Products']['name']['pic_thumb'], $_FILES['Products']['tmp_name']['pic_thumb'], $width, $height, USERFILES . '/products', $this->title . '-thumb');
			}
			if ($_FILES['Products']['name']['pic_full']) {
				...
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_full = $file->processUpload($_FILES['Products']['name']['pic_full'], $_FILES['Products']['tmp_name']['pic_full'], $width, $height, USERFILES . '/products', $this->title);
			}
		} else {
			...
			if ($_FILES['Products']['name']['pic_thumb']) {
				...
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_thumb = $file->processUpload($_FILES['Products']['name']['pic_thumb'], $_FILES['Products']['tmp_name']['pic_thumb'], $width, $height, USERFILES . '/products', $this->title . '-thumb', $this->_oldImage_thumb);
			} else {
				//remove picthumb
			...
			if ($_FILES['Products']['name']['pic_full']) {
				...
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_full = $file->processUpload($_FILES['Products']['name']['pic_full'], $_FILES['Products']['tmp_name']['pic_full'], $width, $height, USERFILES . '/products', $this->title, $this->_oldImage_full);
			} else {
