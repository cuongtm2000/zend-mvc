...
<?php if($listItemIndex['models']):?>
	...
	<?php foreach($listItemIndex['models'] as $value):?>
		<li>
			<h3 class="title-items-news"><a href="<?php echo Yii::app()->request->baseUrl . LANGURL . '/' . Yii::t('user', $this->module->id . '.link') . '/' . $value[ucfirst($this->module->id).'Cat']['tag'.LANG] ?>/<?php echo $value['tag'.LANG] ?>.html" title='<?php echo $value['title'.LANG] ?>'><?php echo $value['title'.LANG] ?></a></h3>
			<div class="frame-img">
