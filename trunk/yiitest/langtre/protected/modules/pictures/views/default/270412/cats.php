...
<ul class="all-product-cat">
    <?php foreach($list_sub_cats as $value): ?>
        <li class="product">
            ...
            <h3 class="title-product"><a href="<?php echo Yii::app()->request->baseUrl.LANGURL ?>/<?php echo Yii::t('user', $this->module->id . '.link')?>/<?php echo $value['tag'.LANG] ?>" title="<?php echo $value['cat_title'.LANG] ?>"><strong><?php echo $value['cat_title'.LANG] ?></strong></a></h3>
        </li>
    <?php endforeach; ?>
</ul>
<?php endif; ?>
    ...
    <ul class="all-product">
        <?php foreach($list_items['models'] as $value): ?>
            <li class="product">
				...
				<a  class="group2" href="<?php echo Yii::app()->baseUrl.USERFILES ?>/<?php echo $this->module->id?>/<?php echo $value['pic_full'] ?>" title="<?php echo $value['title'.LANG] ?>">
				...
				<h3 class="title-product"><a class="group1" href="<?php echo Yii::app()->baseUrl.USERFILES ?>/<?php echo $this->module->id?>/<?php echo $value['pic_full'] ?>" title="<?php echo $value['title' . LANG] ?>"><?php echo $value['title' . LANG] ?></a></h3>
            </li>
        <?php endforeach; ?>
    </ul><!--End All products-->
