...
class Pictures extends CActiveRecord {
	...
	/*public function checkExistsTag($attribute) {
		if (GetTag::tag($this->tag, $this->record_id, $this->tableName(), 'id', 2, 'dos_module_pictures_cat')) {
	...
	public function relations() {
		...
		return array(
			'PicturesCat' => array(self::BELONGS_TO, 'PicturesCat', 'dos_module_item_cat_cat_id'),
		);
	}
	...
	public function beforeSave() {
		...
		if ($this->isNewRecord) {
			...
			if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_thumb']) {
				...
				$this->pic_thumb = $file->processUpload($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_thumb'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_thumb'], Configs::configTemplate('pictures_width_thumb', Yii::app()->session['template']), Configs::configTemplate('pictures_height_thumb', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title . '-thumb');
			}
			if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full']) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_full = $file->processUpload($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_full'], Configs::configTemplate('pictures_width', Yii::app()->session['template']), Configs::configTemplate('pictures_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title);
			}
			//pic_desc
			if (isset($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_desc'])) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_desc = implode("|", $file->uploadMulti($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_desc'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_desc'], Configs::configTemplate('pictures_width', Yii::app()->session['template']), Configs::configTemplate('pictures_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title));
			}
		} else {
			...
			if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_thumb']) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_thumb = $file->processUpload($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_thumb'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_thumb'], Configs::configTemplate('pictures_width_thumb', Yii::app()->session['template']), Configs::configTemplate('pictures_height_thumb', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title . '-thumb', $this->_oldImage_thumb);
			} else {
				//remove picthumb
			...
			if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full']) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_full = $file->processUpload($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_full'], Configs::configTemplate('pictures_width', Yii::app()->session['template']), Configs::configTemplate('pictures_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title, $this->_oldImage_full);
			} else {
				$this->pic_full = $this->_oldImage_full;
			}
			//check remove pic_desc
			if (isset($_POST['Pictures']['remove_pic_desc'])) {
				$str = explode('|', $this->_oldImage_desc);
				foreach ($_POST['Pictures']['remove_pic_desc'] as $value) {
					$common_class = new Common();
			...
			if (isset($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_desc'])) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$uploaded = $file->uploadMulti($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_desc'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_desc'], Configs::configTemplate('pictures_width', Yii::app()->session['template']), Configs::configTemplate('pictures_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title);
	...
	public function listItemNew() {
		...
		$criteria->limit = Configs::configTemplate('pictures_num_paging_new', Yii::app()->session['template']);
	...
	public function listItemHot() {
		...
		$criteria->limit = Configs::configTemplate('pictures_num_paging_hot', Yii::app()->session['template']);
	...
	public function listItemByCat($cid) {
		$criteria = new CDbCriteria();
		$criteria->select = 'title' . LANG . ', pic_thumb, tag' . LANG . ',pic_full, unit,record_id';
		$criteria->order = 'record_order DESC, postdate DESC';
		...
		$pages->pageSize = Yii::app()->controller->configs['pictures_num_paging_cat'];
	...
	public function listItemOther($id, $cid) {
		...
		$criteria->limit = Yii::app()->controller->configs['pictures_num_paging_other'];
	...
	public function deleteRecord($id) {
		$item = Pictures::model()->find('record_id=:id', array(':id' => $id));
		...
		Pictures::model()->findByPk($id)->delete(); //delete record_id
	}
	...
	public function activeItem($data) {
		...
		if ($sort) {
			...
			$criteria->alias = 'p';
			$criteria->join = 'JOIN dos_module_pictures_cat c ON p.dos_module_item_cat_cat_id = c.cat_id';
			$criteria->condition = 'dos_usernames_username=:user';
			$criteria->params = array(':user' => Yii::app()->user->id);

			$models = Pictures::model()->findAll($criteria);

			$i = 1;
			foreach ($models as $model) {
				Pictures::model()->updateByPk($model['record_id'], array('record_order' => $i));
				$i++;
			}
		} else {
			if (!empty($ids)) {
	...
	public function maxRecordOrder() {
		...
		$command = Yii::app()->db->createCommand('SELECT max(record_order) AS max FROM ' . $this->tableName() . ', dos_module_pictures_cat WHERE dos_module_pictures_cat.cat_id = ' . $this->tableName() . '.dos_module_item_cat_cat_id AND dos_usernames_username=:user');
