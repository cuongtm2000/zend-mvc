 ...
 * @property DosModuleFood[] $dosModuleNews
...
class FoodCat extends CActiveRecord {
	...
	public function relations() {
		...
		return array(
			'Food' => array(self::HAS_MANY, 'Food', 'dos_module_item_cat_cat_id'),
	...
	public function countItemCat($id) {
		$user = Yii::app()->user->id;
		$command = Yii::app()->db->createCommand('SELECT COUNT(record_id) AS numcat FROM ' . $this->tableName() . ', dos_module_food WHERE ' . $this->tableName() . '.cat_id=dos_module_food.dos_module_item_cat_cat_id AND cat_id=:id AND dos_usernames_username=:user');
		$command->bindParam(":id", $id, PDO::PARAM_INT);
		$command->bindParam(":user", $user, PDO::PARAM_STR);
		return $command->queryScalar();
	}
