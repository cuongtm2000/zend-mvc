...
class TemplatesBussiness extends CActiveRecord {
	...
	public function listBusinessByTemplateforEdit($template) {
		$data = array();
		$rows = $this->listBusinessByTemplate($template);
		foreach($rows as $row){
			$data[] = $row['dos_bussiness_bussiness_id'];
		}
		return $data;
	}

	private function listBusinessByTemplate($template) {
		$command = Yii::app()->db->createCommand('SELECT dos_bussiness_bussiness_id FROM ' . $this->tableName() . ' WHERE dos_templates_template=:template');
