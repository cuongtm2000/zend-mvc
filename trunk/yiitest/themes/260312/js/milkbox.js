...
var Milkbox = new Class({
	...
	openMilkbox:function(gallery,index){
		...
		if(gallery.length == 1){
			...
			this.description.setStyle('margin-right',navWidth);//alert(this.description.innerHTML );
			this.buttonimages.innerHTML ="";
			for(b_img=gallery.length-1;b_img>=0;b_img--){
				
				temp  = new Element('div',{'id':'mbButtonImages' + b_img}).setStyle('visibility','visible');
				if(index==b_img)temp.className="button_active";
				else temp.className="button";
				temp.innerHTML = b_img + 1 ;
				temp.addEvent('click',this.images_selected.bindWithEvent(this,b_img));
				this.buttonimages.adopt(temp);
	...
	next_prev_aux:function(e,direction){
		...
		var i, _i;
		$("mbButtonImages" + this.currentIndex).className="button";
		if(direction == "next"){
		...
		$("mbButtonImages" + i).className="button_active" ;
		this.canvas.empty();
	...
	images_selected:function(e,index){
		...
		if (index != this.currentIndex ){ 
			$("mbButtonImages" + this.currentIndex).className="button"; 
			$("mbButtonImages" + index).className="button_active" ;
			this.currentIndex=  index ;
			i = index;
		}
		else return ;
		...
		_i = this.currentIndex + 1; if(_i>this.currentGallery.length-1) _i = 0;
	...
	prepareHTML:function(){		
		...
		this.buttonimages = new Element('div',{'id':'mbButtonImages'}).setStyle('visibility','visible');

		this.bottom.adopt(this.navigation, this.buttonimages, this.description, new Element('div',{'class':'mbClear'}));
		
		this.close = new Element('a',{'id':'mbCloseLink'});
		this.next = new Element('a',{'id':'mbNextLink'}).setStyle('visibility','visible');;
		this.prev = new Element('a',{'id':'mbPrevLink'}).setStyle('visibility','visible');;
