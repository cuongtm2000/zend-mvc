    ...
    <ul id="all-product">
        <?php foreach($list_items['models'] as $value): ?>
            <li class="product">
                ...
                <a href="<?php echo Yii::app()->request->baseUrl . LANGURL . '/' . Yii::t('user', $this->module->id . '.link') . '/' . $info_cat['tag'.LANG] ?>/<?php echo $value['tag'.LANG] ?>.html" title="Add to cart"class="add-to-cart">Add to cart</a>
            </li>
        <?php endforeach; ?>
    </ul><!--End All products-->
