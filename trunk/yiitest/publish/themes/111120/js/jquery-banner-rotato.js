	...
	$.fn.wtRotator = function(params) {
		...
		function VertStripes(rotator, areaWidth, areaHeight, stripeSize, bgColor, duration, delay) {
			...
			var animateRandom = function($img) {					
				var count = 0;		
				for (var i = 0; i < total; i++) {
					$($stripes.get(i)).delay(Math.random() * duration).animate({opacity:1}, duration, "",
						function() {
							count++;
							if (count == total) {
								rotator.setComplete($img);
		...
		function Blocks(rotator, areaWidth, areaHeight, blockSize, bgColor, duration, delay) {
			...
			var init = function() {
				...
				for (var i = 0; i < numRows; i++) {
					...
					for (var j = 0; j < numCols; j++) {
						$blockArr[i][j] = $("div#" + (i + "-" + j), rotator.$el);
					}
				}
			...
			this.displayContent = function($img, effect) {
				switch (effect) {
					...
					case EFFECTS["block.bottom"]:
						setBlocks($img, 1, 0, 0);
						horzAnimate($img, effect);
						break;
					default:
						setBlocks($img, 1, 0, 0);
						vertAnimate($img, effect);					
				}
			}
			...
			var setBlocks = function($img, opacity, size, tPos) {
				...
				for (var i = 0; i < numRows; i++) {							
					for (var j = 0; j < numCols; j++) {
						...
						$blockArr[i][j].css({"background":bgColor + " url('"+ imgSrc +"') " + lVal + "px " + tVal + "px no-repeat",
				...
				intervalId = setInterval(
					function() {
						...
						while(i >= 0) {			
							...
							$blockArr[i][j].animate(props, duration, "",
								function() {
			...
			var vertAnimate = function($img, effect) {
				var j, incr, end, lastId;
				if (effect == EFFECTS["block.left"]) {
					j = 0;
					end = numCols - 1;
					incr = 1;
					lastId = $blocks.data("brId");
				...
				intervalId = setInterval(
					function() {
						for (var i = 0; i < numRows; i++) {
							$blockArr[i][j].animate({opacity:1, width:blockSize, height:blockSize}, duration, "",
								function() {
									if ($(this).attr("id") == lastId) {
										rotator.setComplete($img);
									}
			...
			var horzAnimate = function($img, effect) {
				var i, incr, end, lastId;
				if (effect == EFFECTS["block.top"]) {
					i = 0;
					incr = 1;
					end = numRows - 1;
					lastId = $blocks.data("brId");
			...
			var randomAnimate = function($img) {
				...
				for (var i = 0; i < numRows; i++) {
					for (var j = 0; j < numCols; j++) {
						$blockArr[i][j].delay(Math.random() * duration).animate({top:(i * blockSize), width:blockSize, height:blockSize, opacity:1}, duration, "",
							function() {
								count++;
								if (count == total) {
									rotator.setComplete($img);
			...
			var spiral = function($img, spiralOut) {
				...
				if ($arr.length > 0) {
					...
					var lastId = $arr[end].attr("id");
					var k = 0;				
					intervalId = setInterval(
						function() {
							$arr[k].animate({opacity:1}, duration, "",
								function() {
									if ($(this).attr("id") == lastId) {
										rotator.setComplete($img);
									}
		...
		function Rotator($obj, opts) {
			...
			this.init = function() {
				...
				vStripes =  new VertStripes(this, screenWidth, screenHeight, getPosNumber(opts.vert_size, 50), bgColor, duration, getPosNumber(opts.vstripe_delay, INTERVAL_DELAY));
				hStripes =  new HorzStripes(this, screenWidth, screenHeight, getPosNumber(opts.horz_size, 50), bgColor, duration, getPosNumber(opts.hstripe_delay, INTERVAL_DELAY));				
				blocks = 	new Blocks(this, screenWidth, screenHeight, getPosNumber(opts.block_size, 100), bgColor, duration, getPosNumber(opts.block_delay, INTERVAL_DELAY));	
			...
			var showTooltip = function(e) {
				$tooltip.html($items[$(this).index()].data("caption"));
				var yOffset = $tooltip.data("bottom") ? $tooltip.data("yOffset") : -($tooltip.outerHeight() + $tooltip.data("yOffset"));
				$tooltip.css({top:e.pageY + yOffset, left:e.pageX - 8}).stop(true, true).delay(TOOLTIP_DELAY).fadeIn(300);
			}
