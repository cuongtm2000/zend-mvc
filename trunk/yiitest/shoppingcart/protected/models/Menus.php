...
class Menus extends CActiveRecord {
	...
	public static function getSeoPage($module) {
		$command = Yii::app()->db->createCommand('SELECT title' . LANG . ', description' . LANG . ' FROM dos_module_menus WHERE url=:url');
		$command->bindParam(":url", $module, PDO::PARAM_STR);
		return $command->queryRow();
	}

	public function setMenu() {
		$command = Yii::app()->db->createCommand('SELECT menu' . LANG . ', url, target FROM ' . $this->tableName() . ' ORDER BY position ASC');
