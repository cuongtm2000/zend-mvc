...
/**
 * This is the model class for table "dos_module_customers_cat".
 *
 * The followings are the available columns in table 'dos_module_customers_cat':
 ...
 * @property DosModuleCustomers[] $dosModuleCustomers
 * @property DosUsernames $dosUsernamesUsername
 */
class CustomersCat extends CActiveRecord {
	//private $_subdomain;
	 ...
	 * @return CustomersCat the static model class
	 */
	...
	public function tableName() {
		return 'dos_module_customers_cat';
	}
	...
	public function relations() {
		...
		return array(
			'customers' => array(self::HAS_MANY, 'customers', 'dos_module_item_cat_cat_id'),
	...
	public function countItemCat($id) {
		$user = Yii::app()->user->id;
		$command = Yii::app()->db->createCommand('SELECT COUNT(record_id) AS numcat FROM ' . $this->tableName() . ', dos_module_customers WHERE ' . $this->tableName() . '.cat_id=dos_module_customers.dos_module_item_cat_cat_id AND cat_id=:id AND dos_usernames_username=:user');
		$command->bindParam(":id", $id, PDO::PARAM_INT);
		$command->bindParam(":user", $user, PDO::PARAM_STR);
		return $command->queryScalar();
	}
	...
	public function deleteRecord($id) {
		...
		/*$item = customersCat::model()->find('cat_id=:id', array(':id' => $id));
		$path = YiiBase::getPathOfAlias('webroot') . USERFILES . '/customersCat/';
		//Del pic_full field
		if (($item->pic_full)) {
