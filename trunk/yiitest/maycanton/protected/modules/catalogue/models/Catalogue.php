...
/**
 * This is the model class for table "dos_module_catalogue".
 *
 * The followings are the available columns in table 'dos_module_catalogue':
    ...
    public function beforeSave() {
        ...
        if ($this->isNewRecord) {
            ...
            if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full']) {
                ...
                $this->pic_full = EUploadedImage::getInstance($this, 'pic_full')->processUpload(Configs::configTemplate('catalogue_width', Yii::app()->session['template']), Configs::configTemplate('catalogue_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title);
            }
        } else {
            ...
            if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full']) {
                //import class upload images
                Yii::import('ext.EUploadedImage.EUploadedImage');
                $this->pic_full = EUploadedImage::getInstance($this, 'pic_full')->processUpload(Configs::configTemplate('catalogue_width', Yii::app()->session['template']), Configs::configTemplate('catalogue_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title, $this->_oldImage);
            } else {
                //remove picthumb
