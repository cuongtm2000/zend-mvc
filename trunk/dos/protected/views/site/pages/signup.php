...
<div class="input_field">
    ...
    <?php echo $form->textField($model, 'username', $htmlOptions = array('class' => 'smallfield-name', 'title' => 'ví dụ: thuonghoi, sau khi bạn đăng ký sẽ có website là: http://thuonghoi.dos.vn Lưu ý không dấu, không có khoảng trắng giữa các ký tự', 'tabindex' => 1)); ?>
    <input class="smallfield-dos" type="text" value=".dos.vn" readonly="readonly"/>
...
<div class="sign-up-heading"><h1><span>2</span>Select a Template</h1>
    <p>Don't worry, you can change your template at any time.</p>
</div>
<div class="border-image">
    <div class="border-heading">
        <strong>Choose from our gallery of over 300 professionally designed templates</strong>
        <div class="right">
        ...
        <div class="belt">
            <?php $model_class = new Templates; $values = $model_class->listTemplates(); foreach ($values as $value): ?>
                <div class="panel">
                    ...
                    <p>
                        <?php $checked = ''; if (isset($_POST['Username']['dos_templates_template']) && $_POST['Username']['dos_templates_template'] == $value['template']) {$checked = ' checked="checked"';} else {$checked = '';} ?>
    ...
    <p id="gallerya-paginate">
        <script type="text/javascript">embedImg('<?php echo Yii::app()->theme->baseUrl; ?>');</script>
    </p>
    ...
    jQuery(function () {
        ...
        jQuery("#Username_choose_business").change(function () {
            jQuery("#Username_choose_business option:selected").each(function () {
                jQuery("#gallerya").html('/site/loadtemplates/id/'+jQuery(this).val());
            });
        });
    });
</script>
