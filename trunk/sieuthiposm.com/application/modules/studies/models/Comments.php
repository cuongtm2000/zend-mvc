<?php
class Studies_Model_Comments extends Zend_Db_Table{
	protected $_name = 'dos_module_studies_comments';
	 ...
	 public function addItem($data=NULL){
		try{
			$bind = array('comment_title' => htmlspecialchars($this->_xss->purify(trim($data['commenttitle']))), 
						  ...
						  'dos_module_studies_record_id' => htmlspecialchars($this->_xss->purify($data['id'])),    				  
	...
	public function listItem($data = NULL){
    	...
    	$select = $this->select()->from($this->_name, 
							   ...
							   ->where('dos_module_studies_record_id = ?',$data['id'])
	...
	public function listItems($data = NULL){
    	...
    	$select = $this->select()->from($this->_name, 
									array('comment_id', 'comment_title', 'comment_content', 'comment_enable','dos_sys_users_username','dos_module_studies_record_id','comment_date'))
	...
	public function delItemFrom($post_id){
        $this->delete('dos_module_studies_record_id='.$post_id);
    }
