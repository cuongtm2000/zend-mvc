...
class Products extends CActiveRecord{
    ...
    public function maxRecordOrder(){
        $command = Yii::app()->db->createCommand('SELECT max(record_order)+1 FROM '.$this->tableName().', dos_module_product_cats WHERE dos_module_product_cats.cat_id = '.$this->tableName().'.dos_module_product_cats_cat_id AND dos_usernames_username=:user');
