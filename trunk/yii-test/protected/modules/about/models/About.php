...
class About extends CActiveRecord{
    ...
    public function beforeSave() {
        ...
        $this->tag = GetTag::tag($purifier->purify($this->tag), $this->record_id, $this->tableName());
    ...
    public function homeRecord(){
        $command = Yii::app()->db->createCommand('SELECT title'.LANG.', content'.LANG.' FROM '.$this->tableName().' WHERE hot=1 AND dos_usernames_username=:user ORDER BY record_order DESC, created DESC');
        $command->bindParam(":user", $this->_subdomain, PDO::PARAM_STR);
        return $command->queryRow();
    }
    //Front end - Get first record
    public function firstRecord(){
        $command = Yii::app()->db->createCommand('SELECT title'.LANG.', content'.LANG.', description FROM '.$this->tableName().' WHERE dos_usernames_username=:user ORDER BY record_order DESC, created DESC');
        $command->bindParam(":user", $this->_subdomain, PDO::PARAM_STR);
        return $command->queryRow();
    }
    ...
    public function maxRecordOrder(){
        $command = Yii::app()->db->createCommand('SELECT max(record_order)+1 FROM '.$this->tableName().' WHERE dos_usernames_username=:user');
        $command->bindParam(":user", $this->_subdomain, PDO::PARAM_STR);
