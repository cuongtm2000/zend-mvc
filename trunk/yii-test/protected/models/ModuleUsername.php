...
class ModuleUsername extends CActiveRecord{
	...
	public function moduleByUser($user){
		$data = array();
        $command = Yii::app()->db->createCommand('SELECT dos_modules_module_id, module_id FROM '.$this->tableName().', dos_modules WHERE '.$this->tableName().'.dos_modules_module_id = dos_modules.module_id AND dos_modules_module_id NOT IN(\'banner\') AND dos_usernames_username=:user');
        $command->bindParam(":user", $user, PDO::PARAM_STR);
		$result = $command->queryAll();
		
		$data['default'] = 'Default';
		foreach($result as $value){
			$data[$value['dos_modules_module_id']] = $value['module_id'];
		}
		return $data;
	}
}