...
// Please remove those attributes that should not be searched.
    ...
    public function activeItem($data) {
        ...
        foreach ($record_id as $value) {
            ...
            if ($flag) {
                ...
                $log = new Log();
                $log->addItem('Thành viên ' . $value . ' được kích hoạt. Người giới thiệu:' . $user['user_gioithieu'], 'Công ty', 'grouplaptrinh', 0, 'gioithieu');
    ...
    private function addToUpperUser($user) {
        ...
        $username = $user['username']; //lưu lại user để gán

        while ($user['user_gioithieu'] != '') {
            $this->debug[]=$user['username'];
            
            
            $user_gioithieu = User::model()->findAllByPk($user['user_gioithieu']);
            $tbl_user_gioithieu = $user_gioithieu->tables;

            if ($user_gioithieu['level'] == $user['level'] + 1) {
                $children = $this->getChildOfUser($user_gioithieu['username']);
                //thêm vào nhánh trái
                $flag = 0;
                if ($user['username'] == $children[0]) {
                    if ($tbl_user_gioithieu->left_child == '') {
                        $tbl_user_gioithieu->left_child == $username;
                        $flag = 1;
                    }
                } elseif (isset($children[1])) {
                    if ($tbl_user_gioithieu->right_child == '') {
                        $tbl_user_gioithieu->right_child == $username;
                        $flag = 1;
                    }
                }
                //nếu đạt chuẩn thì update thông tin bàn
                if ($flag == 1) {
                    if ($tbl_user_gioithieu->right_child != '' && $tbl_user_gioithieu->left_child != '') {
                        $tbl_user_gioithieu->upgrade_date = date("Y-m-d H:i:s");
                        $tbl_user_gioithieu->priority = $tbl_user_gioithieu->getMaxPriority() + 1;
                    }
                    $tbl_user_gioithieu->save();
                }
            }
            $user = $user_gioithieu;
