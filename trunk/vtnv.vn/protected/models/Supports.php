...
class Supports extends CActiveRecord {
	...
	public function rules() {
		...
		return array(
			array('support_name, support_value, support_order, support_type', 'required'),
			array('support_order', 'numerical', 'integerOnly' => true),
			array('support_name, support_phone, support_value', 'length', 'max' => 45),
	...
	public function listItem() {
		$command = Yii::app()->db->createCommand('SELECT support_name, support_phone, support_value, support_order, support_type FROM ' . $this->tableName() . ' ORDER BY support_order ASC');
		return $command->queryAll();
	}
	//Back end - list item admin
	public function listItemAdmin() {
		$command = Yii::app()->db->createCommand('SELECT support_id, support_name, support_phone, support_value, support_order, support_type FROM ' . $this->tableName());
		return $command->queryAll();
	}
	//Back end - Add item
	...
	public function deleteRecord() {
		$command = Yii::app()->db->createCommand('DELETE FROM ' . $this->tableName());
		$command->execute();
	}
}