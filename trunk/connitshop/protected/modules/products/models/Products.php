...
class Products extends CActiveRecord {
    ...
    public function listItemByCat($cid) {
        ...
        // elements per page
        $pages = new CPagination($count);
        $pages->pageSize = Config::getValue('products_num_item_cat');
        $pages->applyLimit($criteria);
    ...
    public function saveRecord($model, $id = null) {
        if (Yii::app()->controller->action->id == 'add') {
            ...
            if (isset($_FILES[__CLASS__ . 'Form']['name']['pic_desc'])) {
                $this->pic_desc = implode("|", $file->uploadMulti($_FILES[__CLASS__ . 'Form']['name']['pic_desc'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_desc'], Config::getValue('products_width_desc'), Config::getValue('products_height_desc'), Yii::getPathOfAlias('filePathUpload') . '/image/' . strtolower(__CLASS__) . '/', $model['title' . Yii::app()->params['default_language']].'-desc-'));
            }
