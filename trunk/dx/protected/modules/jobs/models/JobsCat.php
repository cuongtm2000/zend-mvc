...
class JobsCat extends CActiveRecord {
    private $_data;
    private $_rows;
    private $_rowsize;
     ...
     * @return JobsCat the static model class
     */
    ...
    public function relations() {
        ...
        return array(
            'JobsCatLanguage' => array(self::HAS_MANY, 'JobsCatLanguage', 'cat_id', 'index' => 'language_id'),
        );
    }
    ...
    public function makeMenu($data, $cat_id, $strOpen = '<ul>', $strClose = '</ul>', $tag = 'li', $subTag = 'ul', $subTagItem = 'li') {
        ...
        foreach ($data as $value) {
            if ($value['cat_parent_id'] == 0) {
                $str .= '<' . $tag . '>' . CHtml::link($value->JobsCatLanguage[Yii::app()->language]['cat_title'], array(Yii::app()->controller->setUrlModule() . '/' . $value->JobsCatLanguage[Yii::app()->language]['tag']), array('title' => $value->JobsCatLanguage[Yii::app()->language]['cat_title']));
    ...
    private function menuRecursive($parent_id, $data, $res = '', $sep = '', $subTag = 'ul', $subTagItem = 'li') {
        ...
        foreach ($data as $v) {
            if ($v['cat_parent_id'] == $parent_id) {
                $re = $sep . '<' . $subTagItem . '>' . CHtml::link($v->JobsCatLanguage[Yii::app()->language]['cat_title'], array(Yii::app()->controller->setUrlModule() . '/' . $v->JobsCatLanguage[Yii::app()->language]['tag']), array('title' => $v->JobsCatLanguage[Yii::app()->language]['cat_title']));
    ...
    private function findCatID($tag) {
        $cat_id = JobsCatLanguage::model()->findCatByTag($tag);
        $temp = $cat_id['cat_id'];
        $result = -1;
    ...
    public function listCats($cid = 0, $prefix = NULL, $type = 0, $id = 0) {
        ...
        foreach ($this->_rows as $key => $value) {
            if ($value['cat_parent_id'] == 0) {
                $this->_data[] = array('cat_id' => $value['cat_id'], 'cat_parent_id' => $value['cat_parent_id'], 'tag' => $value->JobsCatLanguage[Yii::app()->language]['tag'], 'cat_title_prefix' => $prefix . $value->JobsCatLanguage[Yii::app()->language]['cat_title'], 'cat_title' => $value->JobsCatLanguage[Yii::app()->language]['cat_title'], 'cat_enable' => $value['cat_enable']);
    ...
    private function loopItem($i, $prefix, $tab = '|-- ') {
        for ($j = 0; $j < $this->_rowsize; $j++) {
            if ($this->_rows[$j]['cat_parent_id'] == $this->_rows[$i]['cat_id']) {
                $this->_data[] = array('cat_id' => $this->_rows[$j]['cat_id'], 'cat_parent_id' => $this->_rows[$j]['cat_parent_id'], 'tag' => $this->_rows[$j]->JobsCatLanguage[Yii::app()->language]['tag'], 'cat_title_prefix' => $prefix . $tab . $this->_rows[$j]->JobsCatLanguage[Yii::app()->language]['cat_title'], 'cat_title' => $this->_rows[$j]->JobsCatLanguage[Yii::app()->language]['cat_title'], 'cat_enable' => $this->_rows[$j]['cat_enable']);
    ...
    public function saveRecord($model, $id = null) {
        if (Yii::app()->controller->action->id == 'addcat') {
            ...
            $this->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('jobs_cat_width_thumb'), Config::getValue('jobs_cat_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['cat_title' . Yii::app()->controller->setting['default_language']]);
            ...
            Yii::import('ext.SimpleImage.CSimpleImage');
            $file = new CSimpleImage();
            $item->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('jobs_cat_width_thumb'), Config::getValue('jobs_cat_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['cat_title' . Yii::app()->controller->setting['default_language']], $item->pic_thumb);

            $item->save();
        }
        JobsCatLanguage::model()->saveRecord($id, $model);
    }
    ...
    private function loopCat($id) {
        ...
        foreach ($result as $value) {
            $this->_sub_cat_num++;
            $this->_sub_num_item += Jobs::model()->count('hoiit_module_item_cat_cat_id=:cat_id', array(':cat_id' => $value['cat_id']));
    ...
    public function loopDelItemtoCat($cat_id) {
        $result = $this->getIdByParentId($cat_id);
        foreach ($result as $value) {
            Jobs::model()->deleteItembyCat($value['cat_id']);
    ...
    public function loopMoveItemtoCat($cat_id, $cat_id_new) {
        $result = $this->getIdByParentId($cat_id);
        foreach ($result as $value) {
            Jobs::model()->updateByPk($value['cat_id'], array('hoiit_module_item_cat_cat_id' => $cat_id_new));
    ...
    public function deleteRecord($id) {
        JobsCatLanguage::model()->deleteRecord($id);
