...
class Urls extends CActiveRecord {
    ...
    public function getLinkLanguage($module, $user) {
        $data = array();
        $command = Yii::app()->db->createCommand('SELECT link_id, language_id FROM ' . $this->tableName() . ' WHERE module_id=:module AND user_id=:user');
        $command->bindParam(":module", $module, PDO::PARAM_STR);
        $command->bindParam(":user", $user, PDO::PARAM_STR);
        $rows = $command->queryAll();

        if (!$rows) {
            return Links::model()->getLinkLanguage($module);
        }
    ...
    /**
     * Function: Xóa các items của Module cho User này -> sau đó copy mới -> và tiến hành thay thế
     * @param $id
     * @param $user
     */
    //Back end - Add item
    public function addItems($id, $url, $link_replace) {
        $items = $this->listByModule($id);
        foreach ($items as $item) {
            foreach ($url as $key => $value) {
                if ($item['hoiit_languages_language_id'] == $key) {
                    $url_pattern = str_replace($link_replace, $value, $item['url_pattern']);
                    $this->updateAll(array('url_pattern' => $url_pattern), 'url_pattern=:pattern AND hoiit_modules_module_id=:module', array(':pattern' => $item['url_pattern'], ':module' => $id));
                }
            }
        }
    }
