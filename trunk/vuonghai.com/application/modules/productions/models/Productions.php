<?php
class Productions_Model_Productions extends Zend_Db_Table{
	protected $_name = 'dos_module_productions';
	...
	public function menuAdmin(){
		$cat = new Productions_Model_ProductionsCat();
        $menu[] = array('url'=>'productions/cat', 'name'=>'Danh mục', 'nameen'=>'Category', 'namefr'=>'Category', 'num'=>$cat->countCat());
        $menu[] = array('url'=>'productions', 'name'=>'Bài viết', 'nameen'=>'Post', 'namefr'=>'Post', 'num'=> $this->countItemadmin());
        $menu[] = array('url'=>'productions/comment', 'name'=>'Comment', 'nameen'=>'Comment', 'namefr'=>'Comment', 'num'=> 0);
        return $menu;
    }
	...
	public function delPic($id){
		...
		if($result['pic_thumb']){
			@unlink(PUBLIC_PATH.'/userfiles/images/productions/'.$result['pic_thumb']);
		}
    }
	...
	public function addItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
    		...
    		$upload->upload($_FILES['file_thumb'], $this->_config['productions_width_thumb'], $this->_config['productions_height_thumb'], $this->_config['sys_size_upload'], PUBLIC_PATH.'/userfiles/images/productions/');
	...
	public function saveItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
	    	...
	    	if($upload->isVail() == false){
				$file_thumb = $upload->transferFile(); //upload file
				$upload->removeFile(PUBLIC_PATH.'/userfiles/images/productions/'.$data['fileold_thumb']);
			}
    	}
