<?php
class Materials_Model_MaterialsCat extends Zend_Db_Table{
	protected $_name = 'dos_module_materials_cat';
    ...
    public function getDetailCatFromCid($cid){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()
    		->from($this->_name, array('cat_id', 'cat_title'.LANG))
    		->where('cat_id = ?', $cid);
		return $db->fetchRow($select);
    }
	...
	public function countItemSub($data = NULL){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from(array('p' => 'dos_module_materials'), array('COUNT(record_id) AS numcat'))
	...
	private function countItembyCat($id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from('dos_module_materials', array('COUNT(record_id) AS numitem'))->where('dos_module_item_cat_cat_id = ?', $id);
		return $db->fetchOne($select);
    }
