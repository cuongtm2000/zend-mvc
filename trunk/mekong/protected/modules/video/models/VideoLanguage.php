...
class VideoLanguage extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('record_id, language_id, title, link, tag, hit', 'required'),
            array('record_id, hit', 'numerical', 'integerOnly' => true),
            array('language_id', 'length', 'max' => 2),
            array('title, link, tag, extra_field1, extra_field2', 'length', 'max' => 100),
            array('description', 'length', 'max' => 250),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('record_id, language_id, title, link, content, tag, description, hit, extra_field1, extra_field2', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function saveRecord($id, $model) {
        foreach (Yii::app()->controller->listLanguage as $key => $value) {
            $this->executeRecord($id, $key, $model['title' . $key], $model['link' . $key], $model['content' . $key], $model['tag' . $key], $model['description' . $key]);
        }
    }

    //Back end - save
    private function executeRecord($id, $lang, $title, $link, $content, $tag, $description) {
        ...
        $command = Yii::app()->db->createCommand('INSERT INTO ' . $this->tableName() . ' (record_id, language_id, title, link, content, tag, description) VALUES (:record_id, :language_id, :title, :link, :content, :tag, :description)');
        if (Yii::app()->controller->action->id == 'edit') {
            $command = Yii::app()->db->createCommand('UPDATE ' . $this->tableName() . ' SET title=:title, link=:link, content=:content, tag=:tag, description=:description WHERE record_id=:record_id AND language_id=:language_id');
        }
    ...
    public function loadEdit($id) {
        $command = Yii::app()->db->createCommand('SELECT language_id, title, link, content, tag, description FROM ' . $this->tableName() . ' WHERE record_id=:id');
        $command->bindParam(":id", $id, PDO::PARAM_INT);
