...
class Product_Model_ProductCat extends Zend_Db_Table{
	...
	public function getListSubCat($data = NULL){
    	$select = $this->select()->from($this->_name, array('cat_id', 'pic_thumb', 'cat_title'.LANG, 'preview'.LANG))->where('cat_parent_id = ?', $data['cid'])->where('cat_enable = 1')->order('cat_order ASC');
    	$result = $this->fetchAll($select);
    	if(count($result)){
    		return $result->toArray();
    	}else{
