<?php
class Handbook_Model_HandbookCat extends Zend_Db_Table{
	protected $_name = 'dos_module_handbook_cat';
	...
	public function countItemSub($data = NULL){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from(array('p' => 'dos_module_handbook'), array('COUNT(record_id) AS numcat'))
							   ->join(array('c' => $this->_name), 'p.dos_module_handbook_cat_cat_id = c.cat_id', array())
							   ->where('cat_id = ?', $data['id']);
