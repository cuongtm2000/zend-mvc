<?php
class Promotions_Model_Promotions extends Zend_Db_Table{
	protected $_name = 'dos_module_promotions';
	...
	public function menuAdmin(){
        $menu[] = array('url'=>'promotions', 'name'=>'Bài viết', 'nameen'=>'Post', 'namefr'=>'Post', 'num'=>$this->countItem());
        return $menu;
    }
    ...
    public function addItem($data = NULL){
    	...
    	if (!empty($_FILES['file']['name'])){
    		...
    		$upload->upload($_FILES['file'], $this->_config['promotions_width_thumb'], $this->_config['promotions_height_thumb'], $this->_config['sys_size_upload'], PUBLIC_PATH.'/userfiles/images/promotions/');
    ...
    public function saveItem($data = NULL){
    	...
    	if (!empty($_FILES['file']['name'])){
	    	...
	    	if($upload->isVail() == false){
				$filename = $upload->transferFile(); //upload file
				$upload->removeFile(PUBLIC_PATH.'/userfiles/images/promotions/'.$data['fileold']);
			}else{
				print_r($upload->_errors);
			}
    	}
	...
	private function delPic($id){
		...
		@unlink(PUBLIC_PATH.'/userfiles/images/promotions/'.$result['pic_full']);
    }
}