...
class GLT_Models_NewsCat extends Zend_Db_Table {
    ...
    public function getListmenu() {
        $select = $this->select()->from($this->_name, array('cat_id', 'cat_title' . LANG, 'cat_parent_id'))
                ->where('cat_enable = 1')
                ->order('cat_order ASC');
    ...
    public function getDetailCat($data = NULL) {
        $select = $this->select()->from($this->_name, array('cat_title' . LANG))
    ...
    public function getCatParent_CatOrder($cid) {
        $select = $this->select()->from($this->_name, array('cat_parent_id', 'cat_order'))
                ->where('cat_id = ?', $cid);
        return $this->fetchRow($select);
    }
