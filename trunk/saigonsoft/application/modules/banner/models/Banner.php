...
class Banner_Model_Banner extends Zend_Db_Table{
	...
	//Front end - Get all Banner
    public function getAllBanner($page){
         $select = $this->select()->from($this->_name, array('banner_id', 'banner_name', 'banner_url', 'banner_link', 'banner_size'))
                                  ->where('enable = ?', 1)
                                  ->where("position = 'banner'")
                                  ->where("page_position = ?", $page)
    ...
    public function addItem($data = NULL){
    	...
    	if (!empty($_FILES['file']['name'])){
	    	...
	    	if($upload->isVail() == false){
				...
				list($width, $height) = $upload->isImages();
			}else{
    ...
    public function listItem($data = NULL){
		...
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('banner_id', 'banner_date', 'banner_name', 'banner_url', 'banner_order', 'enable', 'position', 'page_position'))
    						   ->order('banner_order DESC')
