<?php
class Solution_Model_SolutionCat extends Zend_Db_Table{
	protected $_name = 'dos_module_solution_cat';
    ...
    public function getDetailCat($data = NULL){
    	...
    	$select = $db->select()->from($this->_name, array('cat_title'.LANG))->where('cat_id = ?', $data['cid']);
	...
	public function countItemSub($data = NULL){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from(array('p' => 'dos_module_solution'), array('COUNT(record_id) AS numcat'))
							   ->join(array('c' => $this->_name), 'p.dos_module_solution_cat_cat_id = c.cat_id', array())
							   ->where('cat_id = ?', $data['id']);
	...
	private function countItembyCat($id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from('dos_module_news', array('COUNT(record_id) AS numitem'))->where('dos_module_news_cat_cat_id = ?', $id);
		return $db->fetchOne($select);
    }
    ...
    public function loopDelItemtoCat($cat_id){
		...
		$item = new Solution_Model_Solution();
		foreach ($result as $value){
