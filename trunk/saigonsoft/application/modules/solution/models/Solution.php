<?php
class Solution_Model_Solution extends Zend_Db_Table{
	protected $_name = 'dos_module_solution';
	...
	public function listNew($num=5){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from(array('n' => $this->_name), array('record_id', 'pic_thumb', 'postdate', 'title'.LANG, 'preview'.LANG))
							   ->join(array('c' => $this->_name.'_cat'), 'n.dos_module_solution_cat_cat_id = c.cat_id', array('cat_title'.LANG))
							   ->where('enable = 1')
	...
	public function listNewsbyCat($data = NULL){
    	...
    	$select = $db->select()->from($this->_name, array('record_id', 'pic_thumb', 'postdate', 'title'.LANG, 'preview'.LANG))
							   ->where('enable = 1')
							   ->where('dos_module_solution_cat_cat_id =?', $data['cid'])
							   ->order('record_order ASC')
							   ->order('postdate DESC')
    ...
    public function countItembyCatID($cid){
		...
		$select = $db->select()->from($this->_name, array('COUNT(record_id) AS num'))
                               ->where('dos_module_solution_cat_cat_id = ?', $cid);
	...
	public function menuAdmin(){
		$cat = new Solution_Model_SolutionCat();
        $menu[] = array('url'=>'solution/cat', 'name'=>'Danh mục', 'nameen'=>'Category', 'namefr'=>'Category', 'num'=>$cat->countCat());
        $menu[] = array('url'=>'solution', 'name'=>'Bài viết', 'nameen'=>'Post', 'namefr'=>'Post', 'num'=> $this->countItemadmin());
        return $menu;
    }
	...
	public function delPic($id){
		...
		if($result['pic_thumb']){
			@unlink(PUBLIC_PATH.'/userfiles/images/solution/'.$result['pic_thumb']);
		}
    }
	...
	public function addItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
    		...
    		$upload->upload($_FILES['file_thumb'], $this->_config['solution_width_thumb'], $this->_config['solution_height_thumb'], $this->_config['sys_size_upload'], PUBLIC_PATH.'/userfiles/images/solution/');
	...
	public function saveItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
	    	...
	    	if($upload->isVail() == false){
				$file_thumb = $upload->transferFile(); //upload file
				$upload->removeFile(PUBLIC_PATH.'/userfiles/images/solution/'.$data['fileold_thumb']);
			}
    	}
	...
	public function delItembyCat($data = NULL){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id'))->where('dos_module_solution_cat_cat_id =?', $data['id']);
		$result = $db->query($select)->fetchAll();
    	...
    	if($data['delproduct'] == 'del'){
	    	...
	    	foreach ($result as $value){
	    		//move item
				$where = 'record_id = '.$value['record_id'];
		    	$this->update(array('dos_module_solution_cat_cat_id' => $data['catmove']), $where);
			}
    	}
    ...
    public function deleteItem($cat_id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from($this->_name, array('record_id'))->where('dos_module_solution_cat_cat_id = ?', $cat_id);
		$result = $db->fetchAll($select);
		foreach ($result as $value){
			$this->delPic($value['record_id']);
		}
		$where = 'dos_module_solution_cat_cat_id = '.$cat_id;
		$this->delete($where);
    }
	//Back end - update item to new cat
    public function updateItem($cat_id, $cat_id_new){
    	$where = 'dos_module_solution_cat_cat_id = '.$cat_id;
		$this->update(array('dos_module_solution_cat_cat_id' => $cat_id_new), $where);
    }
}