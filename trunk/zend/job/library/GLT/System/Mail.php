...
class GLT_System_Mail{
	...
	public function sendMailLocal($from, $to, $namefrom, $nameto, $subject, $content){
		...
		$mail->addBcc('webcontact@grouplaptrinh.com', $nameto);
		$mail->setSubject($subject);
		$mail->send();
	}
	
	public function sendMailSmtp($from, $to, $namefrom, $nameto, $subject, $content){
		$connmail = new Zend_Mail_Transport_Smtp ( 'smtp.gmail.com', array ('auth' => 'login', 'username' => 'noreply@grouplaptrinh.com', 'password' => 'Grouplaptrinh', 'ssl' => 'ssl', 'port' => 465 ) );
