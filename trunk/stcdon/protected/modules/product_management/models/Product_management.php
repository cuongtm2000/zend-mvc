 ...
 *
 * The followings are the available columns in table 'hoiit_module_product_management':
 ...
 */
class Product_management extends CActiveRecord {
    private $_model;
    ...
    public function tableName() {
        return 'hoiit_module_product_management';
    }
    ...
    public function relations() {
        ...
        return array(
            'Product_managementCat' => array(self::BELONGS_TO, 'Product_managementCat', 'hoiit_module_item_cat_cat_id'),
            'Language' => array(self::MANY_MANY, 'Language', 'hoiit_module_product_management_languages(record_id, language_id)'),
            'Product_managementLanguage' => array(self::HAS_MANY, 'Product_managementLanguage', 'record_id', 'index' => 'language_id'),
        );
    }
    ...
    public function listItemsNew() {
        return Product_managementCat::model()->listCatFirst();
    ...
    public function listItemsHot() {
        ...
        $criteria->limit = Config::getValue('product_management_num_item_hot');
    ...
    public function listItemsIndex() {
        ...
        $pages->pageSize = Config::getValue('product_management_num_item_index');
    ...
    public function listItemByCat($cid) {
        ...
        // elements per page
        $pages = new CPagination($count);
        $pages->pageSize = Config::getValue('product_management_num_item_cat');
        $pages->applyLimit($criteria);
    ...
    private function deleteRecord($id) {
        Product_managementLanguage::model()->deleteRecord($id);
    ...
    public function saveRecord($model, $id = null) {
        if (Yii::app()->controller->action->id == 'add') {
            ...
            //$this->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('product_mana_width_thumb'), Config::getValue('product_mana_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]);
            ...
            /*Yii::import('ext.SimpleImage.CSimpleImage');
            $file = new CSimpleImage();
            $item->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('product_mana_width_thumb'), Config::getValue('product_mana_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']], $item->pic_thumb);*/

            $item->save();
        }
        Product_managementLanguage::model()->saveRecord($id, $model);
    }

    //Back end - Get record to Edit
    public function loadEdit($id) {
        $command = Yii::app()->db->createCommand('SELECT pic_thumb, hot, enable, field1, hoiit_module_item_cat_cat_id FROM ' . $this->tableName() . ' WHERE record_id=:id');
