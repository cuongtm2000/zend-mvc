<?php
class GLT_Model_Cat extends Zend_Db_Table{
	protected $_module;
	protected $_model;
	protected $_name ;
    ...
    public function getDetailCat($data = NULL){
		$sql = "SELECT cat_title".LANG.", tag FROM `".$this->_name."` WHERE cat_id =?";
        return $this->_db->fetchRow($sql, array($this->getRecordByTag($data['id'])));
	
		/*$sql = "`".$this->_name."`";
    	$select = $this->select()->from($sql, array('cat_title'.LANG, 'tag'))->where('cat_id = ?', $this->getRecordByTag($data['id']));
		echo $select;*/
	...
	public function addItem($data = NULL){		
    	...
    	$data = array('cat_parent_id' => $this->_xss->purify($data['parent_id']), 'cat_title' => $this->_xss->purify(stripslashes($data['cat_title'])), 'cat_titleen' => $this->_xss->purify(stripslashes($data['cat_titleen'])), 'cat_titlefr' => $this->_xss->purify(stripslashes($data['cat_titlefr'])), 'tag' => $this->_xss->purify($data['tag']), 'description' => $this->_xss->purify($data['description']), 'cat_order' => $max_record);
    	$this->insert($data);
    }
	...
	public function saveItem($data = NULL){
    	$where = 'cat_id = '.$data['id'];
    	$data = array('cat_parent_id' => $this->_xss->purify($data['parent_id']), 'cat_title' => $this->_xss->purify(stripslashes($data['cat_title'])), 'cat_titleen' => $this->_xss->purify(stripslashes($data['cat_titleen'])), 'cat_titlefr' => $this->_xss->purify(stripslashes($data['cat_titlefr'])), 'tag' => $this->_xss->purify($data['tag']), 'description' => $this->_xss->purify($data['description']),);
