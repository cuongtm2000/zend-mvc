<?php
class Productions_Model_ProductionsCat extends Zend_Db_Table{
	protected $_name = 'dos_module_productions_cat';
	...
	public function countItemSub($data = NULL){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from(array('p' => 'dos_module_productions'), array('COUNT(record_id) AS numcat'))
	...
	private function countItembyCat($id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from('dos_module_productions', array('COUNT(record_id) AS numitem'))->where('dos_module_item_cat_cat_id = ?', $id);
		return $db->fetchOne($select);
    }
