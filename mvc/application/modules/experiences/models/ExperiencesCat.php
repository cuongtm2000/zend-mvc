<?php
class Experiences_Model_ExperiencesCat extends Zend_Db_Table{
	protected $_name = 'dos_module_experiences_cat';
	...
	public function countItemSub($data = NULL){
		$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from(array('p' => 'dos_module_experiences'), array('COUNT(record_id) AS numcat'))
							   ->join(array('c' => $this->_name), 'p.dos_module_experiences_cat_cat_id = c.cat_id', array())
							   ->where('cat_id = ?', $data['id']);
	...
	private function countItembyCat($id){
    	$db = Zend_Registry::get('connectDb');
    	$select = $db->select()->from('dos_module_experiences', array('COUNT(record_id) AS numitem'))->where('dos_module_experiences_cat_cat_id = ?', $id);
		return $db->fetchOne($select);
    }
