...
/**
 * This is the model class for table "dos_module_pictures_cat".
 *
 * The followings are the available columns in table 'dos_module_pictures_cat':
 ...
 * @property DosModulePictures[] $dosModulePictures
...
class PicturesCat extends CActiveRecord {
	...
	public function tableName() {
		return 'dos_module_pictures_cat';
	}
	...
	public function rules() {
		...
		return array(
			...
			array('cat_id, cat_parent_id, cat_title, cat_titleen, preview, previewen, tag, tagen, description, descriptionen, pic_full, pic_desc, cat_order, cat_extra1, cat_extra2, cat_enable, dos_usernames_username', 'safe', 'on' => 'search'),
		);
	}
	...
	public function relations() {
		...
		return array(
			'Pictures' => array(self::HAS_MANY, 'Pictures', 'dos_module_item_cat_cat_id'),
	...
	public function attributeLabels() {
		return array(
			...
			'pic_full' => Yii::app()->controller->lang['pictures'],
	...
	public function listCats($cid = 0, $prefix = NULL, $type = 0, $id = 0) {
		...
		if ($type == 1) {
			//for admin
			if ($id != 0) {
				$command = Yii::app()->db->createCommand('SELECT cat_id, cat_parent_id, cat_title' . LANG . ', tag' . LANG . ', pic_full, cat_enable FROM ' . $this->tableName() . ' WHERE cat_id != ' . $id . ' ORDER BY cat_order DESC');
			} else {
				$command = Yii::app()->db->createCommand('SELECT cat_id, cat_parent_id, cat_title' . LANG . ', tag' . LANG . ', pic_full, cat_enable FROM ' . $this->tableName() . ' ORDER BY cat_order DESC');
			}
		} else {
			$command = Yii::app()->db->createCommand('SELECT cat_id, cat_parent_id, cat_title' . LANG . ', tag' . LANG . ', pic_full, cat_enable FROM ' . $this->tableName() . ' WHERE cat_enable=1 ORDER BY cat_order DESC');
		}
		...
		for ($i = 0; $i < $this->_rowsize; $i++) {
			if ($this->_rows[$i]['cat_parent_id'] == 0) {
				$this->_data[] = array('cat_id' => $this->_rows[$i]['cat_id'], 'cat_parent_id' => $this->_rows[$i]['cat_parent_id'], 'tag' . LANG => $this->_rows[$i]['tag' . LANG], 'cat_title_prefix' => $prefix . $this->_rows[$i]['cat_title' . LANG], 'cat_title' . LANG => $this->_rows[$i]['cat_title' . LANG], 'pic_full' => $this->_rows[$i]['pic_full'], 'cat_enable' => $this->_rows[$i]['cat_enable']);
	...
	/*
	   ...
	   public function listItemAdmin($cid = 0) {
		   ...
		   $count = PicturesCat::model()->count($criteria);
		   ...
		   $this->_cat_data = PicturesCat::model()->findAll($criteria);
	...
	public function activeItem($data) {
		...
		if ($syn) {
			...
			$models = PicturesCat::model()->findAll($criteria);

			$i = 1;
			foreach ($models as $model) {
				PicturesCat::model()->updateByPk($model['cat_id'], array('cat_order' => $i));
				$i++;
			}
		} else {
			if (!empty($ids)) {
	...
	public function loadEdit($id) {
		...
		$this->_model = PicturesCat::model()->findByPk($id, $criteria);
	...
	public function countItemCat($id) {
		$user = Yii::app()->user->id;
		$command = Yii::app()->db->createCommand('SELECT COUNT(record_id) AS numcat FROM ' . $this->tableName() . ', dos_module_pictures WHERE ' . $this->tableName() . '.cat_id=dos_module_pictures.dos_module_item_cat_cat_id AND cat_id=:id AND dos_usernames_username=:user');
		$command->bindParam(":id", $id, PDO::PARAM_INT);
		$command->bindParam(":user", $user, PDO::PARAM_STR);
		return $command->queryScalar();
	}
	...
	public function deleteRecord($id) {
		...
		/*$item = PicturesCat::model()->find('cat_id=:id', array(':id' => $id));
		$path = YiiBase::getPathOfAlias('webroot') . USERFILES . '/PicturesCat/';
		//Del pic_full field
		if (($item->pic_full)) {
