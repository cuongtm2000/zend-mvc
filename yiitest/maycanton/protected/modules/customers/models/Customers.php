...
class Customers extends CActiveRecord {
	...
	/*public function checkExistsTag($attribute) {
		if (GetTag::tag($this->tag, $this->record_id, $this->tableName(), 'id', 2, 'dos_module_customers_cat')) {
	...
	public function relations() {
		...
		return array(
			'CustomersCat' => array(self::BELONGS_TO, 'CustomersCat', 'dos_module_item_cat_cat_id'),
		);
	}
	...
	public function beforeSave() {
		...
		if ($this->isNewRecord) {
			...
			if ($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full']) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$this->pic_full = $file->processUpload($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_full'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_full'], Configs::configTemplate('customers_width', Yii::app()->session['template']), Configs::configTemplate('customers_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title, $this->_oldImage_full);
			} else {
			...
			if (isset($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_desc'])) {
				Yii::import('ext.simpleImage.CSimpleImage');
				$file = new CSimpleImage();
				$uploaded = $file->uploadMulti($_FILES[ucfirst(Yii::app()->controller->id)]['name']['pic_desc'], $_FILES[ucfirst(Yii::app()->controller->id)]['tmp_name']['pic_desc'], Configs::configTemplate('customers_width', Yii::app()->session['template']), Configs::configTemplate('customers_height', Yii::app()->session['template']), USERFILES . '/' . Yii::app()->controller->id, $this->title);
	...
	public function listItemHot() {
		...
		$criteria->limit = Configs::configTemplate('customers_num_paging_hot', Yii::app()->session['template']);
	...
	public function listItemByCat($cid) {
		...
		$pages->pageSize = Yii::app()->controller->configs['customers_num_paging_cat'];
	...
	public function activeItem($data) {
		...
		if ($sort) {
			...
			$criteria->alias = 'p';
			$criteria->join = 'JOIN dos_module_customers_cat c ON p.dos_module_item_cat_cat_id = c.cat_id';
			$criteria->condition = 'dos_usernames_username=:user';
			$criteria->params = array(':user' => Yii::app()->user->id);
	...
	public function maxRecordOrder() {
		...
		$command = Yii::app()->db->createCommand('SELECT max(record_order) AS max FROM ' . $this->tableName() . ', dos_module_customers_cat WHERE dos_module_customers_cat.cat_id = ' . $this->tableName() . '.dos_module_item_cat_cat_id AND dos_usernames_username=:user');
