...
/**
 * This is the model class for table "dos_module_features".
 *
 * The followings are the available columns in table 'dos_module_features':
...
class Features extends CActiveRecord {
    ...
    public function beforeSave() {
        ...
        if ($this->isNewRecord) {
            //Add new record
            $this->record_order = $this->maxRecordOrder(Yii::app()->user->id);
     ...
     * @return mixed
     */
    public function listTopHome($num) {
        $command = Yii::app()->db->createCommand('SELECT title' . LANG . ', tag' . LANG . ' FROM ' . $this->tableName() . ' WHERE activated = 1 ORDER BY record_order DESC, created DESC LIMIT :num, 4');
    ...
    public function maxRecordOrder($user) {
