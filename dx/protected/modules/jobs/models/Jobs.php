...
class Jobs extends CActiveRecord {
    ...
    public function relations() {
        ...
        return array(
            'JobsCat' => array(self::BELONGS_TO, 'JobsCat', 'hoiit_module_item_cat_cat_id'),
            'JobsLanguage' => array(self::HAS_MANY, 'JobsLanguage', 'record_id', 'index' => 'language_id'),
        );
    }
    ...
    public function detailItem($tag) {
        $id = JobsLanguage::model()->getIDByTag($tag);
        $criteria = new CDbCriteria();
    ...
    public function listItemOther($id, $cid) {
        ...
        $criteria->limit = Config::getValue('jobs_num_item_other');
    ...
    public function listItemsNew() {
        return JobsCat::model()->listCatFirst();
    }
    ...
    public function listItemsHot() {
        ...
        $criteria->limit = Config::getValue('jobs_num_item_hot');
    ...
    public function listItemsIndex() {
        ...
        $pages->pageSize = Config::getValue('jobs_num_item_index');
    ...
    public function listItemByCat($cid) {
        ...
        // elements per page
        $pages = new CPagination($count);
        $pages->pageSize = Config::getValue('jobs_num_item_cat');
        $pages->applyLimit($criteria);
    ...
    private function deleteRecord($id) {
        JobsLanguage::model()->deleteRecord($id);
    ...
    public function saveRecord($model, $id = null) {
        if (Yii::app()->controller->action->id == 'add') {
            ...
            $this->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('jobs_width_thumb'), Config::getValue('jobs_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']]);
            ...
            Yii::import('ext.SimpleImage.CSimpleImage');
            $file = new CSimpleImage();
            $item->pic_thumb = $file->processUpload($_FILES[__CLASS__ . 'Form']['name']['pic_thumb'], $_FILES[__CLASS__ . 'Form']['tmp_name']['pic_thumb'], Config::getValue('jobs_width_thumb'), Config::getValue('jobs_height_thumb'), '/image/' . lcfirst(__CLASS__), $model['title' . Yii::app()->controller->setting['default_language']], $item->pic_thumb);

            $item->save();
        }
        JobsLanguage::model()->saveRecord($id, $model);
    }
