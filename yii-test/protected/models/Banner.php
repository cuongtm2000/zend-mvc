...
class Banner extends CActiveRecord{
	...
	public function rules(){
		...
		return array(
			...
			array('banner_order, enable', 'numerical', 'integerOnly'=>true),
			array('banner_name, banner_url, dos_usernames_username', 'length', 'max'=>45),
            array('banner_url', 'file', 'types'=>'gif,png,jpg,jpeg,icon', 'allowEmpty'=>true, 'maxSize'=>1024*1024*5),
    ...
    public function beforeSave() {
        ...
        if (!$this->isNewRecord){
            if($_FILES[ucfirst(Yii::app()->controller->id)]['name']['banner_url']){
                ...
                $this->banner_url = EUploadedImage::getInstance($this, 'banner_url')->processUpload(Configs::configTemplate('banner_width', Yii::app()->session['template']), Configs::configTemplate('banner_height', Yii::app()->session['template']), Yii::app()->controller->id, $this->_oldImage);
            }else{
            ...
            if($_FILES[ucfirst(Yii::app()->controller->id)]['name']['banner_url']){
                //import class upload images
                Yii::import('ext.EUploadedImage.EUploadedImage');
                $this->banner_url = EUploadedImage::getInstance($this, 'banner_url')->processUpload(Configs::configTemplate('banner_width', Yii::app()->session['template']), Configs::configTemplate('banner_height', Yii::app()->session['template']), Yii::app()->controller->id);
            }
        }
    ...
    public function getBanner($type, $module){
        $command = Yii::app()->db->createCommand('SELECT banner_name, banner_url, banner_link FROM '.$this->tableName().' WHERE banner_type=:type AND position=:module AND enable=1 AND dos_usernames_username=:user');
    ...
    public function positionBanner(){
        ...
        return $model->moduleByUser($this->_subdomain);
    }
    //Back end - Get max record
    public function maxRecordOrder(){
        $command = Yii::app()->db->createCommand('SELECT max(banner_order)+1 FROM '.$this->tableName().' WHERE dos_usernames_username=:user');
        $command->bindParam(":user", $this->_subdomain, PDO::PARAM_STR);
