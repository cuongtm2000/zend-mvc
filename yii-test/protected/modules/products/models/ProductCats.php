...
/**
 * This is the model class for table "dos_module_product_cats".
 *
 * The followings are the available columns in table 'dos_module_product_cats':
 ...
 */
class ProductCats extends CActiveRecord{
    private $_cat_data;
	 ...
	 * @return ProductCats the static model class
	 */
	...
	public function tableName(){
		return 'dos_module_product_cats';
	}
	...
	public function relations(){
		...
		return array(
			...
			'Products' => array(self::HAS_MANY, 'Products', 'dos_module_product_cats_cat_id'),
		);
	}
    ...
    public function beforeSave() {
        ...
        if (!$this->isNewRecord){
            if($_FILES['ProductCats']['name']['pic_full']){
                Yii::import('ext.simpleImage.CSimpleImage');
                $file = new CSimpleImage($_FILES['ProductCats']['name']['pic_full'], $_FILES['ProductCats']['tmp_name']['pic_full'], Configs::configTemplate('product_cats_width', Yii::app()->session['template']), Configs::configTemplate('product_cats_height', Yii::app()->session['template']), 'productCats', $this->_oldImage_full);
            ...
            $this->dos_usernames_username = $this->_subdomain;
            if($_FILES['ProductCats']['name']['pic_full']){
                Yii::import('ext.simpleImage.CSimpleImage');
                $file = new CSimpleImage($_FILES['ProductCats']['name']['pic_full'], $_FILES['ProductCats']['tmp_name']['pic_full'], Configs::configTemplate('product_cats_width', Yii::app()->session['template']), Configs::configTemplate('product_cats_height', Yii::app()->session['template']), 'productCats');
                $this->pic_full = $file->getFilename();
            }
        }
    ...
    private function maxRecordOrder(){
        $command = Yii::app()->db->createCommand('SELECT max(cat_order)+1 FROM '.$this->tableName().' WHERE dos_usernames_username=:user');
        $command->bindParam(":user", $this->_subdomain, PDO::PARAM_STR);
    ...
    public function countItemCat($id){
        $command = Yii::app()->db->createCommand('SELECT COUNT(record_id) AS numcat FROM '.$this->tableName().', dos_module_products WHERE '.$this->tableName().'.cat_id=dos_module_products.dos_module_product_cats_cat_id AND cat_id=:id AND dos_usernames_username=:user');
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $command->bindParam(":user", $this->_subdomain, PDO::PARAM_STR);
        return $command->queryScalar();
	}
