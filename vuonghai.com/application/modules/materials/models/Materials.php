<?php
class Materials_Model_Materials extends Zend_Db_Table{
	protected $_name = 'dos_module_materials';
	...
	public function menuAdmin(){
		$cat = new Materials_Model_MaterialsCat();
        $menu[] = array('url'=>'materials/cat', 'name'=>'Danh mục', 'nameen'=>'Category', 'namefr'=>'Category', 'num'=>$cat->countCat());
        $menu[] = array('url'=>'materials', 'name'=>'Bài viết', 'nameen'=>'Post', 'namefr'=>'Post', 'num'=> $this->countItemadmin());
        $menu[] = array('url'=>'materials/comment', 'name'=>'Comment', 'nameen'=>'Comment', 'namefr'=>'Comment', 'num'=> 0);
        return $menu;
    }
	...
	public function delPic($id){
		...
		if($result['pic_thumb']){
			@unlink(PUBLIC_PATH.'/userfiles/images/materials/'.$result['pic_thumb']);
		}
    }
	...
	public function addItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
    		...
    		$upload->upload($_FILES['file_thumb'], $this->_config['materials_width_thumb'], $this->_config['materials_height_thumb'], $this->_config['sys_size_upload'], PUBLIC_PATH.'/userfiles/images/materials/');
	...
	public function saveItem($data = NULL){
    	...
    	if (!empty($_FILES['file_thumb']['name'])){
	    	...
	    	if($upload->isVail() == false){
				$file_thumb = $upload->transferFile(); //upload file
				$upload->removeFile(PUBLIC_PATH.'/userfiles/images/materials/'.$data['fileold_thumb']);
			}
    	}
