...
class MenuMultiLevel extends Menu {
    ...
    private function getItems($parent_id = null) {
        $items = array();
        $results = Yii::app()->db->createCommand()->select('hoiit_module_menus.menu_id, menu_name, menu_type, menu_url')->from('hoiit_module_menus')->join('hoiit_module_menus_languages', 'hoiit_module_menus.menu_id=hoiit_module_menus_languages.menu_id');
        ...
        foreach ($results AS $result) {
            ...
            $items[] = array(
                ...
                'url' => ($result['menu_type']==0) ? $result['menu_url'] : Yii::app()->baseUrl . '/' . $result['menu_url'],
                //'url' => Yii::app()->baseUrl . '/' . $result['menu_url'],
                //'itemOptions' => array('class' => 'listItem'),
                'linkOptions' => array( /*'class' => 'listItemLink', */
                    'title' => $result['menu_name'], 'target' => ($result['menu_type']==0) ? '_blank' : null),
