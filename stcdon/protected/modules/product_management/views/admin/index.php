        ...
        <tbody>
            <?php if ($models): ?>
                <?php $i = 1; foreach ($models as $value): ?>
					...
					<?php if($value['hot'] == 1){$title = '<strong>'.$value->Product_managementLanguage[Yii::app()->language]['title'].'</strong> <img src="'.Yii::app()->theme->baseUrl.'/images/hot.gif" alt="Hot" />'; }else{$title = $value->Product_managementLanguage[Yii::app()->language]['title'];}?>
                    <tr<?php echo $rowclass ?>>
                        ...
                        <td><?php echo $value->Product_managementCat->Product_managementCatLanguage[Yii::app()->language]['cat_title'] ?></td>
                        <td><?php echo ($value['field1']=='da-dat-cho') ? 'Đã đặt chỗ' : (($value['field1']=='da-giao-dich') ? 'Đã giao dịch' : 'Chưa giao dịch') ?></td>
